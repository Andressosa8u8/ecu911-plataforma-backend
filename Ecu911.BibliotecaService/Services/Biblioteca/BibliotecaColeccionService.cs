...
public class BibliotecaColeccionService : IBibliotecaColeccionService
{
    private readonly IBibliotecaColeccionRepository _repository;
    private readonly IOrganizationalUnitRepository _organizationalUnitRepository;
    private readonly AuditService _auditService;

    public BibliotecaColeccionService(
        IBibliotecaColeccionRepository repository,
    ...
    public async Task<List<BibliotecaColeccionDto>> GetAllAsync()
    {
        var items = await _repository.GetAllAsync();
        return items.Select(MapToDto).ToList();
    }

    public async Task<List<BibliotecaColeccionDto>> GetRootsAsync()
    {
        var items = await _repository.GetRootsAsync();
        return items.Select(MapToDto).ToList();
    }

    public async Task<List<BibliotecaColeccionDto>> GetChildrenAsync(Guid parentId)
    {
        var items = await _repository.GetChildrenAsync(parentId);
        return items.Select(MapToDto).ToList();
    }

    public async Task<BibliotecaColeccionDto?> GetByIdAsync(Guid id)
    {
        var item = await _repository.GetByIdAsync(id);
        return item == null ? null : MapToDto(item);
    }

    public async Task<BibliotecaColeccionDto> CreateAsync(CreateBibliotecaColeccionDto input, string? username)
    {
        ...
        var entity = new BibliotecaColeccion
        {
        ...
        _auditService.LogAction(
            "Create",
            username ?? "Unknown",
            $"Created BibliotecaColeccion with name: {created.Name}");

        return MapToDto(created);
    }

    public async Task<BibliotecaColeccionDto?> UpdateAsync(Guid id, UpdateBibliotecaColeccionDto input, string? username)
    {
        ...
        _auditService.LogAction(
            "Update",
            username ?? "Unknown",
            $"Updated BibliotecaColeccion with ID: {id}");
    ...
    public async Task<bool> DeleteAsync(Guid id, string? username)
    {
        ...
        if (deleted)
        {
            _auditService.LogAction(
                "Delete",
                username ?? "Unknown",
                $"Deleted BibliotecaColeccion with ID: {id}");
        }

        return deleted;
    }

    private static BibliotecaColeccionDto MapToDto(BibliotecaColeccion x)
    {
        return new BibliotecaColeccionDto
        {
            Id = x.Id,
            Name = x.Name,
            Code = x.Code,
