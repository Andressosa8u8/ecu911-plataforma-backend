...
public class BibliotecaDocumentosController : ControllerBase
{
    private readonly IBibliotecaDocumentoService _service;
    private readonly IBibliotecaArchivoService _documentFileService;
    private readonly IBibliotecaDescargaService _downloadAuditService;

    public BibliotecaDocumentosController(
        IBibliotecaDocumentoService service,
        IBibliotecaArchivoService documentFileService,
        IBibliotecaDescargaService downloadAuditService)
    {
        _service = service;
        ...
        [FromQuery] int pageSize = 10)
    {
        ...
        var filter = new BibliotecaDocumentoFilterDto
        {
            Title = title,
            BibliotecaCategoriaId = documentTypeId,
            BibliotecaColeccionId = repositoryNodeId,
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateBibliotecaDocumentoDto input)
    {
        try
        {
    ...
    [Authorize(Roles = "ADMIN,GESTOR_DOCUMENTAL")]
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateBibliotecaDocumentoDto input)
    {
        try
        {
            var username = UserContextHelper.GetUsername(User);
    ...
    public async Task<IActionResult> DownloadFile(Guid id, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var username = UserContextHelper.GetUsername(User);
            await _downloadAuditService.RegisterAsync(id, result.BibliotecaArchivoId, username);
